...
along with the current team id and context object. The event will also receive the key used for storage, and the cache key for that setting.
...
> {note} This event **will not be fired** if the setting exists and the value is not changed.
